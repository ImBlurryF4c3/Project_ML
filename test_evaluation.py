...
def test_models(DT, LT, DE, LE):
    ...
    for idx, model in enumerate(classifiers):
        ...
        for p in priors:
            ...
            mact = evaluation.compute_actual_DCF(scores, LE, p, 1, 1)
...
def test_best_3_models(DT, LT, DE, LE):
    ...
    for ip in p:
        ...
        actDCF_gmm.append(evaluation.compute_actual_DCF(scores_gmm, LE, ip, 1, 1))
        minDCF_lr.append(evaluation.minimum_DCF(scores_lr, LE, ip, 1, 1))
        actDCF_lr.append(evaluation.compute_actual_DCF(scores_lr, LE, ip, 1, 1))
        minDCF_rbfsvm.append(evaluation.minimum_DCF(scores_rbfsvm, LE, ip, 1, 1))
        actDCF_rbfsvm.append(evaluation.compute_actual_DCF(scores_rbfsvm, LE, ip, 1, 1))
    ...
    for t in types_gmm:         # 2 plot, in base al tipo gmm e gmm tied
        ...
        for title in data_type:
            if title == 'z_score_data':
                ...
                DER = utility.Znormalization(DE) # dati normalizzati
            else:
                DTR = D
                DER = DE
            for c in components:
