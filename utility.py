...
# -------- CARICAMENTO DATI E POSSIBILI SPLITTING --------
...
def plotDCF_evalVSval(x, y, z, xlabel):
    plt.figure()
    print("prior=0.5 ---> minDCF = %.3f for l = %.6f" %(min(y[0:len(x)]), x[numpy.argmin(y[0:len(x)])]))
    plt.plot(x, y[0:len(x)], label='min DCF prior=0.5 (EVAL)', color='b', linestyle='--')
    plt.plot(x, z[0:len(x)], label='min DCF prior=0.5 (VAL)', color='b')
    print("prior=0.1 ---> minDCF = %.3f for l = %.6f" % (min(y[len(x):2*len(x)]), x[numpy.argmin(y[len(x):2*len(x)])]))
    plt.plot(x, y[len(x): 2*len(x)], label='min DCF prior=0.1 (EVAL)', color='r', linestyle='--')
    plt.plot(x, z[len(x): 2*len(x)], label='min DCF prior=0.1 (VAL)', color='r')
    print("prior=0.9 ---> minDCF = %.3f for l = %.6f" % (min(y[2*len(x):3*len(x)]), x[numpy.argmin(y[2*len(x):3*len(x)])]))
    plt.plot(x, y[2*len(x): 3*len(x)], label='min DCF prior=0.9 (EVAL)', color='g', linestyle='--')
    plt.plot(x, z[2*len(x): 3*len(x)], label='min DCF prior=0.9 (VAL)', color='g')
    plt.xlim([min(x), max(x)])
    plt.xscale("log")
    plt.legend(loc='best')
    plt.xlabel(xlabel)
...
def bayesErrorPlot(actDCF, minDCF, effPriorLogOdds, title):
    plt.figure()
    plt.plot(effPriorLogOdds, actDCF, label='actDCF', color='salmon')
    plt.plot(effPriorLogOdds, minDCF, label='minDCF', color='dodgerblue', linestyle='--')
